...
export function ComandaTicket({ pedido }: { pedido: PedidoParaComanda }) {
  const comidas = [...pedido.comidas_pedido].sort(
    (a, b) => a.numero_comida - b.numero_comida
  );
  const diaLabel = pedido.dia_entrega === "miercoles" ? "Miércoles" : "Jueves";

  return (
    <div className="print:break-inside-avoid bg-white border-2 border-black rounded-xl overflow-hidden w-full">
      {/* Encabezado */}
